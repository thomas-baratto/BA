...
\begin{tikzpicture}
    ...
    \node[sum node, diamond, minimum size=5mm, inner sep=0pt, font=\tiny] (cat) at (8.00,0) {cat};
    \node[output node] (out_0) at (9.50,0.90) {};
    \node[annot, right=2.5mm of out_0] {$y_1$};
    \node[output node] (out_1) at (9.50,0.00) {};
    \node[annot, right=2.5mm of out_1] {$y_2$};
    \node[output node] (out_2) at (9.50,-0.90) {};
    \node[annot, right=2.5mm of out_2] {$y_3$};
    \node[lbl, above=5mm] at (9.50,0.90) {Output\\3};
    ...
    \draw[random arr] (H1.east) -- (cat.west);
    ...
    \node[annot, above=1mm] at ($0.5*(in_0.north)+0.5*(cat.north)+(0,1.50)$) {\textit{direct link}};
\end{tikzpicture}
\end{document}
