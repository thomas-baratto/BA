...
\begin{tikzpicture}
    ...
    % Single random hidden layer
    \node[frozen block] (H0) at (3.20,0) {100\\ReLU};
    \node[lbl, above=5mm] at (H0.north) {Random frozen};

    % Concat node
    \node[sum node, diamond, minimum size=5mm, inner sep=0pt, font=\tiny] (cat) at (5.60,0) {cat};

    % Output nodes
    \node[output node] (out_0) at (7.60,0.90) {};
    \node[annot, right=2.5mm of out_0] {$y\_1$};
    \node[output node] (out_1) at (7.60,0.00) {};
    \node[annot, right=2.5mm of out_1] {$y\_2$};
    \node[output node] (out_2) at (7.60,-0.90) {};
    \node[annot, right=2.5mm of out_2] {$y\_3$};
    \node[lbl, above=5mm] at (7.60,0.90) {Output\\3};
    \node[annot, below=3mm] at (cat.south) {Ridge regression};

    % Arrows: input -> hidden (random frozen)
    \draw[random arr] (in_0) -- (H0.west);
    \draw[random arr] (in_1) -- (H0.west);
    \draw[random arr] (in_2) -- (H0.west);
    \draw[random arr] (in_3) -- (H0.west);

    % Arrow: hidden -> concat (random)
    \draw[random arr] (H0.east) -- (cat.west);

    % Arrows: concat -> output (learned)
    \draw[learned arr] (cat.east) -- (out_0);
    \draw[learned arr] (cat.east) -- (out_1);
    \draw[learned arr] (cat.east) -- (out_2);

    % Direct link (input bypass arc)
    \draw[direct arr] (in_0.north) to[out=80, in=100] (cat.north);
    \node[annot, above=1mm] at ($0.5*(in_0.north)+0.5*(cat.north)+(0,1.35)$) {\textit{direct link}};
    ...
    % title (commented out to match other generic diagrams)
%     \node[font=\bfseries, above=8mm] at (current bounding box.north) {RVFL};
\end{tikzpicture}
\end{document}
