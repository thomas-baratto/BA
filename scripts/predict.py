...
Usage:
  ...
  python scripts/predict.py --input data/new_samples.csv --dataset isotherm --model mlp

  # Predict cone using random model (ELM)
  python scripts/predict.py --input data/new_samples.csv --dataset cone --model random

  # Specify output location
  python scripts/predict.py --input data.csv --dataset isotherm --model mlp --output results/
...
DEFAULT_MODELS = {
    "isotherm": {
        "mlp": "data/good_runs/mlp_final/isotherm_MLP_*",
        "random": "data/good_runs/run_training_random_943/isotherm_ELM_*",
    },
    "cone": {
        "mlp": "data/good_runs/mlp_final/cone_MLP_*",
        "random": "data/good_runs/run_training_random_943/cone_ELM_*",
    },
}
...
def predict_mlp(model, scalers, config, features: np.ndarray) -> np.ndarray:
    """Run MLP prediction."""
    ...
    def predict_with_model(trained_model: TrainedModel, config: dict, features: np.ndarray) -> np.ndarray:
        ...
        # Handle square root for area in random models
...
def main():
    ...
    if args.model_dir:
        ...
        pattern = DEFAULT_MODELS[args.dataset][args.model]
        try:
    ...
    try:
        if args.model == "mlp":
            model, scalers, config = load_mlp_model(model_dir)
        else:
            model, scalers, config = load_random_model(model_dir)
    except FileNotFoundError as e:
    ...
    if args.model == "mlp":
        predictions = predict_mlp(model, scalers, config, features)
    else:
        predictions = predict_random(model, scalers, config, features)
        # Load model using unified wrapper
        try:
