...
### Installation
Set up your virtual environment and install this package:
```bash
python3 -m venv .venv && source .venv/bin/activate && pip install ./code
```
### Verify Installation
Run the automated validation suite to verify the package installation and model weights integrity, this will also unzip the pre-trained models if they are not already unzipped:
```bash
ba-predict --test
```
### Usage
...
Run prediction on sample depression cone dataset:
```bash
...
```bash
ba-predict --input data/sample_isotherm.csv --model randomized:SResdRVFL
```
Detailed information on all flags can be found with:
```bash
ba-predict --help
```
