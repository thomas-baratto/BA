...
def build_objective(
    ...
    _ = base_log_dir  # TensorBoard logging inside objective is intentionally disabled
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        # --- 2. Suggest Scaler Choices and their Parameters ---
        ...
        def make_scaler_with_params(kind: str, is_label: bool = False):
            k = (kind or '').lower()
            if k == 'minmax':
                r = (label_min, label_max) if is_label else (feature_min, feature_max)
                return MinMaxScaler(feature_range=r)
            if k == 'standard':
        ...
        # --- 4. Setup Data Loaders (CRITICAL FIX: num_workers=0) ---
        ...
        test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False,
                                num_workers=0, pin_memory=True) # Used for final test score
        ...
        # --- 6. Final Evaluation and Logging (Robust against NaN) ---
        
        final_test_rmse = np.nan
        wall_time = time.time() - start_time
        
        try:
            # Final evaluation on the unseen test set (unbiased generalization score)
            _, test_predictions, test_true_values = evaluate(model, test_loader, criterion, DEVICE)
            
            # CRITICAL SAFETY CHECK: Ensure RMSE calculation is clean
            temp_rmse = root_mean_squared_error(test_true_values, test_predictions)
            if not np.isnan(temp_rmse) and not np.isinf(temp_rmse):
                final_test_rmse = temp_rmse
            else:
                final_test_rmse = best_val_rmse # Fallback to a valid value for logging stability

        except Exception as e:
            # If evaluation fails completely (e.g., due to model instability), fall back
            logging.warning(f"Final test RMSE calculation failed: {e}. Logging validation RMSE as test value.")
            final_test_rmse = best_val_rmse 
        
        # --- Log User Attributes (Metadata) ---
        
        trial.set_user_attr("slurm_procid", os.environ.get('SLURM_PROCID', 'N/A'))
        trial.set_user_attr("gpu_device", DEVICE.type if DEVICE.type != 'cuda' else f"cuda:{os.environ.get('CUDA_VISIBLE_DEVICES', 'N/A')}")
        trial.set_user_attr("trial_wall_time_sec", float(f"{wall_time:.2f}"))

        # Core HPs (from suggestion logic)
        trial.set_user_attr("batch_size", int(batch_size))
        trial.set_user_attr("nr_hidden_layers", int(nr_hidden_layers))
        trial.set_user_attr("activation_name", str(activation_name))
        trial.set_user_attr("loss_criterion", str(loss_name))
        trial.set_user_attr("weight_decay", float(weight_decay))
        trial.set_user_attr("dropout_rate", float(dropout_rate))
        
        # New Performance Metric
        trial.set_user_attr("final_test_rmse", float(final_test_rmse))

        # Scaler Choices (as per original logic)
        trial.set_user_attr("feature_scaler", str(feature_scaler))
        trial.set_user_attr("label_scaler", str(label_scaler))
